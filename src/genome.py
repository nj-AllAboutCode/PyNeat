...
class Genome:
    ...
    def add_node(self):
        ...
        for n in self.nodes:
            if node.lay > conn.from_node.lay:
                node.lay += 1
    ...
    def fullyConnected(self):
        ...
        nodes_perlayer = []
        for i in self.nodes:
            if len(nodes_perlayer) > i.lay:
                nodes_perlayer[i.lay] += 1
            else:
                nodes_perlayer.append(0)
    ...
    def add_connection(self):
        if not self.fullyConnected():
            ...
            while ((n1.lay == n2.lay) or self.nodes_connected(n1, n2)) and not self.fullyConnected:
                n1, n2 = random.choice(self.nodes), random.choice(self.nodes)
            if n1.lay > n2.lay:
                temp = n1
                n1 = n2
