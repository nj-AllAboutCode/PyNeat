...
class NodeGene:
    ...
    def activate(self, x):
        if self.activation == 1:  # identity
            return x
        elif self.activation == 2:  # Step
            if x > 0:
                return 1
            else:
                return 0
        elif self.activation == 3:  # tanh
            return math.tanh(x)
        elif self.activation == 4:  # relu
            if x < 0:
                return 0
            else:
                return x
    ...
    def mutateActivation(self):
        self.activation = random.randint(0, 4)
    ...
    def engage(self):
        ...
        if self.lay != 0:
            self.output_val = self.activate(self.input_sum + self.bias)
